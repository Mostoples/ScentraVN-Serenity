...
(() => {
  ...
  const RecordHistory = {
    ...
    async load() {
      ...
      host.innerHTML = `<div class="rh-state"><i class="fas fa-spinner fa-spin" style="font-size:1.6rem;color:#c4b5fd;margin-bottom:12px;"></i><p>Memuat riwayat…</p></div>`;
      let items = [];
      try { items = await RawRecorder.listRecordings(); }
      catch (e) {
        host.innerHTML = `<div class="rh-state"><i class="fas fa-triangle-exclamation bigic" style="color:#fca5a5;"></i><p style="color:#b91c1c;">Gagal memuat riwayat.<br><span style="font-size:0.72rem;">${this._esc(e.message)}</span></p><button class="rh-btn" onclick="RecordHistory.load()"><i class="fas fa-rotate"></i> Coba lagi</button></div>`;
        return;
      }
      if (!items.length) {
        host.innerHTML = `<div class="rh-state">
            <i class="fas fa-folder-open bigic"></i>
            <p>Belum ada rekaman tersimpan.<br>Rekam data mentah perangkat lalu simpan ke cloud.</p>
            <button class="rh-btn primary" onclick="Router.navigate('rawrecorder')"><i class="fas fa-record-vinyl"></i> Mulai Rekam</button>
        </div>`;
        return;
      }
        ...
        <div class="rh-card" data-rec="${it.id}">
            <div class="rh-card-top">
                ...
                <div class="rh-card-hd">
                    <div class="rh-card-name" title="${this._esc(it.name || 'Rekaman')}">${this._esc(it.name || 'Rekaman')}</div>
                ...
                <span class="rh-frames">${this._fmtNum(it.total || 0)} frame</span>
            </div>
            <div class="rh-chips">${chips || '<span class="rh-chip" style="color:#94a3b8;background:#f1f5f9;">tidak ada data stream</span>'}</div>
            <div class="rh-card-foot">
                <button class="rh-dl hist-xlsx" data-id="${it.id}" type="button"><i class="fas fa-file-excel"></i> Download Excel</button>
                <button class="rh-ico-btn hist-json" data-id="${it.id}" type="button" title="Unduh JSON"><i class="fas fa-file-code"></i></button>
                <button class="rh-ico-btn danger hist-del" data-id="${it.id}" type="button" title="Hapus"><i class="fas fa-trash"></i></button>
            </div>
        </div>`;
    },
    ...
    async _fetch(id) {
      ...
      if (!full) throw new Error('Rekaman tidak ditemukan');
      return full;
    },
    ...
    async _downloadExcel(btn, it) {
      if (typeof XLSX === 'undefined') { alert('Library Excel belum termuat. Cek koneksi internet.'); return; }
      const orig = btn.innerHTML; btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Menyusun…';
      try {
        const { meta, streams } = await this._fetch(it.id);
        ...
        const info = [
          ['Nama', it.name || ''],
          ['ID', it.id],
          ['Dibuat', this._fmtDate(it.createdAt) || ''],
          ['Mulai', it.startedAt || ''],
          ['Selesai', it.endedAt || ''],
          ['Durasi (detik)', it.durationSec || 0],
          ['Total frame', it.total || 0],
          ['EEG mentah (paket)', (it.counts || {}).museRaw || 0],
          ['Muse metrik', (it.counts || {}).muse || 0],
          ['ScentraVN', (it.counts || {}).scentra || 0],
          ['Galaxy Watch', (it.counts || {}).galaxy || 0],
        ];
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['Field', 'Value'], ...info]), 'Info');
        ...
        if (wb.SheetNames.length === 1) {
          XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([['(tidak ada data stream)']]), 'Empty');
        }
        XLSX.writeFile(wb, `${this._slug(it.name)}-${it.id}.xlsx`);
      } catch (e) { console.error(e); alert('Gagal membuat Excel: ' + e.message); }
      btn.disabled = false; btn.innerHTML = orig;
    },
    ...
    async _delete(id) {
      if (!confirm('Hapus rekaman ini beserta seluruh data mentahnya? Tindakan tidak dapat dibatalkan.')) return;
    ...
    _fmtNum(n) { return (Number(n) || 0).toLocaleString('id-ID'); },
    _fmtBytes(b) {
      b = Number(b) || 0;
    ...
    _fmtISO(iso) { if (!iso) return null; try { return new Date(iso).toLocaleString('id-ID'); } catch (_) { return null; } },
    _fmtDate(ts) {
      if (!ts) return null;
      try { const d = ts.toDate ? ts.toDate() : new Date(ts); return d.toLocaleString('id-ID'); }
      catch (_) { return null; }
    },
  };
